...
if (browser) {
	...
	if (
		...
		UserAgent.lacks('Chrome') &&
		UserAgent.lacks('Chromium') &&
		UserAgent.lacks('Android')
	)
		DeviceBrowserName = UserAgent.contains('CriOS')
			? 'Chrome for iOS'
	...
	if (UserAgent.contains('Chrome'))
		if (UserAgent.matches(/\bChrome\/[.0-9]* Mobile\b/))
			DeviceBrowserName =
				UserAgent.matches(/\bVersion\/\d+\.\d+\b/) || UserAgent.matches(/\bwv\b/)
					? 'WebView on Android'
					: 'Chrome for Android';
		else DeviceBrowserName = 'Chrome';
	if (
		UserAgent.contains('Android') &&
		UserAgent.lacks('Chrome') &&
		UserAgent.lacks('Chromium') &&
		UserAgent.lacks('Trident') &&
		UserAgent.lacks('Firefox')
	)
		DeviceBrowserName = 'Android Browser';
	if (UserAgent.contains('Edge')) DeviceBrowserName = 'Edge';
	if (UserAgent.contains('UCBrowser')) DeviceBrowserName = 'UC Browser for Android';
	if (UserAgent.contains('SamsungBrowser')) DeviceBrowserName = 'Samsung Internet';
	if (UserAgent.contains('OPR') || UserAgent.contains('Opera'))
		DeviceBrowserName = UserAgent.contains('Opera Mini')
			? 'Opera Mini'
	...
	DeviceBrowserVersion = (VersionMatch && VersionMatch[1]) || '(n/a)';
	/**** DeviceOSName ****/
	// based on: https://github.com/keithws/browser-repor

	if (UserAgent.contains('Windows'))
		DeviceOSName = UserAgent.contains('Windows Phone') ? 'Windows Phone' : 'Windows';
	if (UserAgent.contains('OS X') && UserAgent.lacks('Android')) DeviceOSName = 'macOS';
	if (UserAgent.contains('Linux') && UserAgent.lacks('Android')) DeviceOSName = 'Linux';
	if (UserAgent.contains('like Mac OS X') && UserAgent.contains('iPhone')) DeviceOSName = 'iOS';
	if (UserAgent.contains('like Mac OS X') && UserAgent.contains('iPad')) {
		const VersionMatch = UserAgent.match(/OS ((\d+[._])+\d+) like Mac OS X/);
	...
	if (UserAgent.contains('CrOS')) DeviceOSName = 'ChromeOS';
	if (UserAgent.contains('BB10')) DeviceOSName = 'BlackBerry';
	if (UserAgent.contains('RIM Tablet OS')) DeviceOSName = 'BlackBerry Tablet OS';
	if (UserAgent.contains('BlackBerry')) DeviceOSName = 'BlackBerryOS';
	/**** DeviceOSVersion ****/
	// based on: https://github.com/keithws/browser-report
	...
	switch (DeviceOSName) {
		...
		case 'Windows Phone':
			if (UserAgent.contains('Win16')) {
				...
				DeviceOSVersion = UserAgent.contains('Windows 98; Win 9x 4.90')
					? 'Millennium Edition'
					: '98';
			} else {
				...
				if (VersionMatch && VersionMatch[1]) {
					switch (VersionMatch[1]) {
						...
						case '6.1':
							...
							break;
						case '6.0':
							DeviceOSVersion = 'Vista';
							break;
						case '5.2':
							DeviceOSVersion = 'Server 2003';
							break;
						case '5.1':
							DeviceOSVersion = 'XP';
							break;
						case '5.01':
							DeviceOSVersion = '2000 SP1';
							break;
						case '5.0':
							DeviceOSVersion = '2000';
							break;
						case '4.0':
							DeviceOSVersion = '4.0';
							break;
